...
This startup path additionally checks the mount, contractual quota, receipts,
...
def check_capacity(volume, state, files, capacity, reserve):
    extra = sum(max(0, f["bytes"] - staged_bytes(state, f)) for f in files)
    used = volume_usage(volume)
    free = min(shutil.disk_usage(volume).free, capacity - used)
    if free < extra + reserve:
        raise BootstrapError(
            f"Insufficient Volume capacity: used={used}, additional={extra}, "
            f"reserve={reserve}, configured_capacity={capacity}. "
            "No files removed and no storage resized.")
...
def provision(volume, files, *, capacity, reserve=10_000_000_000,
    ...
    if capacity <= 0 or reserve < 0:
        raise BootstrapError("Set MODEL_VOLUME_CAPACITY_GB to the actual contracted capacity")
    ...
    with volume_lock(state / "bootstrap.lock", lock_timeout):
        ...
        try:
            ...
            if missing:
                check_capacity(volume, state, missing, capacity, reserve)
            for index, f in enumerate(missing):
                check_capacity(volume, state, missing[index:], capacity, reserve)
                stage = stage_for(state, f)
...
def main():
    ...
    try:
        ...
        provision(args.volume_root, files,
                  capacity=int(os.environ.get("MODEL_VOLUME_CAPACITY_GB", "0")) * 1_000_000_000,
